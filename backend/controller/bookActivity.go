...
func CreateBookActivity(c *gin.Context) {
    ...
    db.First(&customer, bookActivity.CustomersID)
    if customer.ID == 0 {
    ...
    a := entity.BookActivity{
		...
		CustomersID: bookActivity.CustomersID,
		Customers:   customer,
...
func GetBookActivity(c *gin.Context) {
	...
	db := config.DB()
	results := db.Preload("Activity").Preload("BookingTrip").Preload("Customers").First(&bookActivity, ID)
...
func ListBookActivitys(c *gin.Context) {
	...
	db := config.DB()
	results := db.Preload("Activity").Preload("BookingTrip").Preload("Customers").Find(&bookActivitys)
	if results.Error != nil {
        if results.Error == gorm.ErrRecordNotFound {
            c.JSON(http.StatusNotFound, gin.H{"error": "BookActivity not found"})
        } else {
...
func GetAllBookActivityByCustomerId(c *gin.Context) {
    ...
    db := config.DB()
	id := c.Param("id")
	if err := db.Preload("BookingTrip").Preload("Customers").Preload("Activity").Raw("SELECT * FROM book_activities WHERE customers_id = ?", id).Find(&bookActivity).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func DeleteBookActivity(c *gin.Context) {
	...
	id := c.Param("id")
	db := config.DB()
	if tx := db.Exec("DELETE FROM book_activities WHERE id = ?", id); tx.RowsAffected == 0 {
